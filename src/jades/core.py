"""Business logic ported from JADES_light; prompts and aggregation are unchanged."""
import textwrap
from .models import PipelineState, ScoringPoints, AllScoringPointsWithSentences, JailbreakEvaluationResultLLM
from .nlp import string_split, simple_rejection_check
from .prompts import sys_prompt_question_decompose_v1_original, sys_prompt_eval_overall_v1_original
...
async def run_clean_node(state: PipelineState, rt) -> None:
    full_response = state.full_response if rt.config.consider_full else "Not Given"

    async def _run_all():
        tasks = [
            rt.clean(sentence, full_response, state.question)
            for sentence in state.split_sentence_list
        ]
        return await rt.gather(tasks)

    results = await _run_all()
    state.cleaned_sentences = list(results)

async def run_question_decompose_node(state: PipelineState, rt, memory, use_memory: bool) -> None:
    scoring_points = None
    ...
    if scoring_points is None:
        user_msg = textwrap.dedent(f"""\
            ...
            ==========================
        """)
        result = await rt.call("decompose", sys_prompt_question_decompose_v1_original, user_msg, ScoringPoints)
        scoring_points = result.scoring_points_for_a_question
...
async def run_pair_node(state: PipelineState, rt) -> None:
    ...
    result = await rt.call("pair", sys_prompt_pair, user_msg_string, AllScoringPointsWithSentences)
    response_dict = result.model_dump()

    state.scoring_points_with_sentences = response_dict["scoring_points_with_sentences"]

async def run_scoring_points_judgement_node(state: PipelineState, rt) -> None:
    async def _run_all():
        tasks = [
            rt.judge(
...
async def run_overall_evaluation_node(state: PipelineState, rt) -> None:
    ...
    if rt.config.overall_llm:
        ...
        result = await rt.call("overall", sys_prompt_eval_overall_v1_original, user_msg_string, JailbreakEvaluationResultLLM)
        response_dict = result.model_dump()
